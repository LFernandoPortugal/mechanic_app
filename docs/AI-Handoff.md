...
- **Rutas migradas:** Recepción, Técnico y Asesor ya usan Precision Blue. Técnico y Asesor comparten el patrón responsive `cola + área de trabajo`, selección semántica, workflow de 8 etapas y enlaces contextuales a Ayuda. Se retiraron sus botones Inicio redundantes, colores de módulo, glows y paneles glass sin cambiar handlers ni transiciones.
...
- **Siguiente paso:** migrar QC, Pagos e Inventario con los mismos componentes/tokens; luego Clientes, ADMIN y SUPER_ADMIN. Después repetir la matriz completa desktop/móvil claro/oscuro, abrir PR y esperar CI + Vercel Preview; no fusionar ni desplegar antes de revisión humana.
