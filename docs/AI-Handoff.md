...
> Código funcional de producción verificado: `e756a14` en `https://mechanic-app-zeta.vercel.app/`
...
- **Producción:** `e756a14` integra los manuales, `/help` por rol ES/EN, Industrial Orange, la identidad SVG adaptativa de SGA y los inicios separados para gestión global y operación del taller; GitHub Actions y Vercel Production finalizaron correctamente.
- **Bloque integrado:** PR #52 se fusionó por squash en `main` como `e756a14`. SUPER_ADMIN ya no entra al resumen del taller y el ADMIN de un taller vacío recibe primeros pasos accionables.
...
- **Último hito integrado:** PR #52 se integró como `e756a14`; añade consola global SUPER_ADMIN, onboarding de taller nuevo y reemplaza el ambiguo `Danger On/Off` por `Habilitar/Deshabilitar borrado`, con indicador destructivo rojo accesible.
- **Calidad verificada:** auditoría runtime sin vulnerabilidades; TypeScript, lint y build de 21 páginas/8 APIs; 129 unitarias, 18 integraciones API y 26 Rules. Los 8 E2E pasaron contra emuladores, incluido `Reception → Diagnosis → Approval → Approved → Repair → QC → Ready → Delivered`, navegación/RBAC, personal, SUPER_ADMIN y la matriz visual de rutas/temas/tamaños. El workflow de `main` `32617057435` repitió todos los gates correctamente en 4m03s.
- **Verificación de producción:** Vercel marcó exitoso el deployment Production del commit `e756a14`. La URL oficial respondió HTTP 200 para `/login`, `/sga-mark.svg` y `/manifest.json`, con MIME `image/svg+xml` y `application/json` en los assets.
- **Siguiente paso:** recoger feedback real sobre la consola global y el onboarding; no hay merge, despliegue manual ni migración pendiente para este bloque.
...
- `origin/main` contiene el código funcional `e756a14`; producción sirve el reset server-side sobre el ciclo ADMIN de personal, la reconciliación global, la consola SUPER_ADMIN, el onboarding de talleres nuevos, los manuales, `/help` por rol, Industrial Orange y la identidad adaptativa de SGA en `https://mechanic-app-zeta.vercel.app/`. El historial anterior de E2E, flujos, portal, accesibilidad y estabilización permanece documentado en Git y en Bugs y Correcciones.
...
8. Un job E2E duplicado falló una vez porque un toast interceptó el clic de cierre de sesión; PR #39 resolvió la superposición y las ejecuciones posteriores pasaron con clics normales.

## Borradores seguros y notificaciones no bloqueantes del 2026-08-12

1. PR #39 se integró por squash en `main` como `8eab8b0`; ambos CI de Preview, CI de `main` y Vercel Preview/Production completaron correctamente.
2. QC conserva orden, checklist, notas y motivo; Caja conserva método, monto y referencia. La misma identidad los recupera al recargar o volver del login en la misma pestaña.
3. Cada borrador queda aislado por función, taller, UID y orden en `sessionStorage`, caduca a los 30 minutos y se elimina al completar o descartar. No persiste tokens, contraseñas ni credenciales.
4. Los valores se validan antes de restaurarse; contenido vencido, corrupto o de otra identidad no entra al formulario ni bloquea la operación principal.
5. Los toasts `top-right` comienzan debajo del header sticky. Los E2E cambian de usuario con clics normales y pasaron localmente, en los dos CI del PR y en `main` sin repetir el bloqueo histórico.
...
2. El restablecimiento operativo ya pasa por API server-side y es reintentable; no volver a introducir cascadas de jobs/movimientos desde el navegador.
3. Coordinar cualquier chat de Diseño mediante una rama `codex/*` distinta; verificar `origin/main`, rebase/merge no destructivo y los gates antes de integrar para evitar solapar archivos de runtime.
4. La variante de pago completo antes de QC ya está cubierta por integración de API y QA histórico; automatizarla en navegador solo si se requiere una regresión visual/estado adicional.
5. Mantener `p1` como fixture de testers hasta finalizar el flujo crítico y limpiar cada orden descartable creada. No repetir despliegues de Rules ni pruebas con SUPER_ADMIN si el siguiente cambio no toca seguridad, roles o datos privilegiados.
6. Mantener aceptados por ahora los 5 avisos moderados dev-only de `firebase-tools`; no aplicar el downgrade automático sugerido por `npm audit --force`.
