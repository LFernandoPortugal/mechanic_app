...
import { BarChart3, Building2, CarFront, CircleHelp, ClipboardPlus, Globe2, Home, LogOut, Menu, Moon, Package, ReceiptText, Settings, ShieldCheck, Sun, Users, Wrench, X } from "lucide-react";
...
const copy = {
  es: { home:"Inicio", reception:"Recepción", technician:"Técnico", advisor:"Órdenes", qc:"Control QC", payments:"Pagos", clients:"Clientes", employees:"Empleados", inventory:"Inventario", analytics:"Analítica", settings:"Configuración", help:"Ayuda", more:"Más", signout:"Cerrar sesión", theme:"Tema", system:"Sistema", close:"Cerrar menú", language:"Cambiar idioma", nav:"Navegación principal", mobileNav:"Navegación móvil" },
  en: { home:"Home", reception:"Reception", technician:"Technician", advisor:"Orders", qc:"Quality check", payments:"Payments", clients:"Clients", employees:"Employees", inventory:"Inventory", analytics:"Analytics", settings:"Settings", help:"Help", more:"More", signout:"Sign out", theme:"Theme", system:"System", close:"Close menu", language:"Change language", nav:"Main navigation", mobileNav:"Mobile navigation" },
} as const;
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const mobilePrimary = visible.filter((item) => ["/", "/reception", "/technician", "/advisor", "/clients", "/inventory"].includes(item.href)).slice(0, 4);
  ...
  return <div className="app-shell min-h-[100dvh] bg-background text-foreground">
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-60 border-r border-sidebar-border bg-sidebar lg:flex lg:flex-col">
      <Link href="/" className="flex h-20 items-center gap-3 border-b border-sidebar-border px-5"><span className="brand-mark"><Building2 size={20}/></span><span className="min-w-0"><strong className="block tracking-tight">Mechanic OS</strong><small className="block truncate text-muted-foreground">{workshopSettings?.workshopName || "SGA"}</small></span></Link>
      <nav className="flex-1 space-y-1 overflow-y-auto p-3" aria-label={c.nav}>{visible.map((item) => renderLink(item))}</nav>
